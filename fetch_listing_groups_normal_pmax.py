        ...
        for row in response:
            ...
            if roas < 7:
                ...
                if normal_camp_product_status == 'UNIT_INCLUDED':
                    ...
                    excluded = exclude_listing_group_main(client, customer_id, normal_pmax_campaign_id, product_id, main_pmax_generic_asset_group_resource_name)
                    ...
                    if excluded and low_perf_camp_product_status == 'UNIT_EXCLUDED':
                        ...
                        added_to_low_performing = include_listing_group_main(client, customer_id, low_performing_campaign_id, product_id, low_performing_generic_asset_group_resource_name)
                ...
                elif normal_camp_product_status == 'UNIT_EXCLUDED':
                    email_body += (
                            f"Campaign -- {row.campaign.name},"
                            f" with asset group name {row.asset_group.name}, "
                            f"has a product: {row.asset_group_listing_group_filter.case_value.product_item_id.value}, "
                            ...
                            f"with a conversion value of ${conversion_value}, "
                            f"and ROAS of ${roas}. \n"
                            f"This listing group NOT been sucessfully excluded in the main pmax campaign, and was not added within the low-performing pmax campaign. \n"
                            f"\n")
                ...
                # handling for status not found
                    ...
                    email_body += (
                            ...
                            f"with a conversion value of ${conversion_value}, "
                            f"and ROAS of ${roas}. \n"
                            f"This listing group status was not found within the main pmax campaign. No actions taken. \n"
                            f"\n")
            # do nothing if roas is meeting the goal
            elif roas >= 7:
                print(f"product {product_id} found, but roas is >= $7")
                email_body += (
                    f"Campaign -- {row.campaign.name},"
                    f" with asset group name {row.asset_group.name}, "
                    f"has a product: {row.asset_group_listing_group_filter.case_value.product_item_id.value}, "
                    ...
                    f"with a conversion value of ${conversion_value}, "
                    f"and ROAS of ${roas}. \n"
                    f"This product is currently meeting it's ROAS goal, and was not moved.\n"
                    f"\n")
